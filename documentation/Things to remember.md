...
- **Safe Lookups & Manipulation:**
  ...
  - **`equals` compares only STRINGS and INTEGERS — never booleans (or other types).** Comparing booleans throws `equals arguments are not the same types`. To check a single flag, use `getCachedValue` (true) / `logicalNOT` (false). To compare TWO boolean variables `a` and `b` (XNOR), use `ifElse(condition=a, thenValue=b, elseValue=logicalNOT(b))`.
...
- **Copying a Cached Var Into a New Var Requires `getCachedValue`:** A `saveValueInCache` entry's top-level `value` must be a literal (`preset`) or a selector (`computed`) — it can NOT be a bare `{"type": "cached", "value": "oldVar"}` reference. To seed `newVar` from an existing `oldVar`, compute it: `"value": {"selector": "getCachedValue", "params": [{"name": "name", "type": "preset", "value": "oldVar"}]}`. (This is the exception to the "prefer type:cached in params" rule above — that rule is about selector *params*, not the entry's top-level value. Serializing the copy as the tuple `["cached","oldVar"]` is a silent bug: the var holds the literal strings, not the value.)
...
- **`createDeck` IMPORTS, `createCustomDeck` BUILDS:** `createDeck` pulls a deck in from the deck store, so its `name` must be the imported deck's script name (rename it locally with `customName`). Every deck the game invents for itself — a discard, a trick pile, a staging area, a pass/tax deck — is a `createCustomDeck`, whose fields are `name` + `public` (plus optional `label`, `counter`, `facedown`, `cardback`). Reaching for `createDeck` with `set: "empty"` and a made-up name is the common slip; the validator's "createDeck.name does not match the script.name" error is usually telling you that you wanted `createCustomDeck`. (Some older games ship the wrong one and the engine tolerates it, so copying them propagates the mistake.)
- **Colour cascades: `visualSettings` is the base, a per-action colour overrides it.** `backgroundColor` / `textColor` / `borderColor` set in `visualSettings` apply to every relevant action; an action setting one of them wins for that action. Both levels work — the trap is overriding them **incompletely**. Give a notification a dark `backgroundColor` and no `textColor` and the text stays at the inherited/default near-black: unreadable, with no error anywhere. So override in complete sets or not at all. Prefer setting the palette once in `visualSettings` — the pattern in the refactored games (Squaffle, Knockout, Braggart), none of which sets a colour on a single notification. Emeralds predates this and paints each notification inline, and its welcome notification has `backgroundColor` + `borderColor` but **no** `textColor` — so recolouring only the keys a lifted block already has silently produces exactly this bug.
...
- **`getCardField` vs `getObjectField` (NOT interchangeable):** `getCardField(cardId, field)` works ONLY inside a `playCards` action, on the played card's ID in that action's result (`lastActionResult.cards.0`); called anywhere else it crashes (`Cannot read properties of undefined (reading 'id')`). Everywhere else, read fields off a card OBJECT (from `getDeckCards`, `playerHand`, `fetchHandField`) with `getObjectField(obj, field)`. Crossing them errors (`getObjectField` on a played-card UUID → `argument is not valid`). Rule: **card ID ⇒ `getCardField` (only in `playCards`); card object ⇒ `getObjectField` (anywhere).** Note `fetchHandField(playerId, "cards")` can return undefined → prefer `playerHand(playerId)` for a reliable list of hand card objects.
- **`getObjectFieldList` Takes `list`, Not `obj`:** `getObjectFieldList`'s first param is `list` (a list of card objects) — it maps over the list returning each element's `field`. Contrast the singular `getObjectField` which takes `obj`. Don't "fix" a `getObjectFieldList` that uses `list`.
- **`createVideoboxDecks` Supports `preset.facedown: true`:** Makes the per-player videobox decks deal face-DOWN (default is face-up). Use it for games that pass a hidden card through videoboxes (Cockroach-Poker-style, e.g. Not My Problem).
...
- **`winnersInfo` (optional top-level section):** Records the *structured* game result alongside — not instead of — `winCondition`/`playersWinCondition`. Those resolve the winner to a display **string**, which is fragile: it breaks when a player renames themselves mid-call and carries no stable identity. `winnersInfo.userIds` instead saves the actual **player IDs** of the winning team (e.g. `getPlayersFromTeam(teamId ← the cached winning team)`), which is what downstream Ludio features should key off. Prefer adding it to new games. *(validator checks the field set and shapes)*
  - Note the deliberate inversion: `playersWinCondition.winners` **must** wrap in `getPlayerNamesByIds` because it is rendered as text; `winnersInfo.userIds` must **not** — names defeat the block's entire purpose.
  - `highScore`/`lowScore` are optional score telemetry (e.g. tracking a game's best score over time) and each must be a single number, not a list. Only include them where scores are comparable **across sessions** — Rainbow Blackjack yes; most card games no, since players play a variable number of rounds, so the raw number says nothing.
  - Fields are bare selector objects — there is no `payload`/`preset`/`cached`/`computed` wrapper, unlike actions. Full field set: `userIds`, `highScore`, `lowScore`.
...
- **Spectator Management:** In `turnSpectatorToPlayerActions` and `turnPlayerToSpectatorActions`, recompute `players` and `numPlayers` using `allPlayers` (not arithmetic). Grant late joiners `currentMinScore` to avoid starting at 0. Place `"turnSpectatorsToPlayers": true` / `"turnPlayersToSpectators": true` on a dedicated empty "Change players" group at end of `gameLoop`. Always end both transition sections with `changeLayout`.
  - **Compute `currentMinScore` at round-end, not in the transition:** `getMinCurrentScore` counts the just-joined spectator's default 0, so calling it inside `turnSpectatorToPlayerActions` returns 0. Cache `currentMinScore = getMinCurrentScore()` at round-end (before the transition) and reference the cached value (pattern: Emeralds).
  - **Over-deal smell:** a spectator who joins mid-game missed the first-round deal, so the transition deals them a starting hand. This is safe with **bulk-recall-then-fixed-deal** refill (`recallCards` the whole deck, then `dealDeck` a fixed N — self-correcting). It DOUBLE-DEALS with **deficit/top-up refill** (`deal(target − currentHandCount)`) because the transition's explicit deal is redundant with it; fix by dropping the transition's explicit deal and letting the deficit refill handle it.
...
- **`<b>` Bold Only Works in `createNotification` text:** Bold/italic tags (`<b>…</b>`, `<i>…</i>`) render as bold ONLY in the `text` field of a `createNotification`. They do NOT render in `createVote`/`createMixVote` questions & titles, `createInput` prompts, `createDrawing` questions, `selectCentralWidgetDeck` questions, or the rulebook (`*_describe.json` rule `content[].text`) — those show the raw tags as literal characters (the describe validator hard-errors on them). Keep the words, drop the tags everywhere except notification text.
...
- **`createDrawing`:** Launches a collaborative whiteboard. Required: `actors`, `question`, `terminationCondition` (`"get_all"`). Key optional: `duration`, `capture` (saves result URL to `lastActionResult.url`), `private`, `vertical`, `colors` (palette list), `tools` (whiteboard implements — a whitelist of `pen`, `eraser`, `circle`, `circleFill`, `rect`, `rectFill`; omit it to keep the default toolset, and note an empty list leaves players nothing to draw with). Supports the sound triplet fields. For multiple simultaneous teams, wrap in `"parallel": {"type": "smart", "qnt": numTeams}` and use `spaIndex` to select the team. Display captured URL via `createNotification` with `preset.image`.
...
- **`takeWidgetScreenshot`:** Captures the central widget and files the image under `title` (required). The uploaded image lands in **`lastActionResult.cloudinaryUrl`** — feed it straight into the next action.
- **`createMoment`:** Records a "moment" between players, saved to a database to build a cross-game feed of in-game moments. Required: `description`, `list1`, `list2`. Optional: `mediaUrl` (normally the screenshot from a `takeWidgetScreenshot`). *(validator checks the field set, list shapes, and mediaUrl provenance)*
  - It writes **one row per pair — `|list1| × |list2|` rows** — so `list1`/`list2` must both be **lists of player IDs**, never scalars. `allPlayers` for both is the common case (and does include self-pairs).
  - Use IDs, not names: wrapping in `getPlayerNamesByIds` writes display strings that break when a player renames mid-call and can't be joined back to a player — the same trap as `winnersInfo.userIds`.
  - **Ordering matters:** `mediaUrl: lastActionResult.cloudinaryUrl` only resolves while `takeWidgetScreenshot` is the *immediately preceding* action. Anything in between — even an `emptyAction` delay — overwrites `lastActionResult`, and the moment is filed with the wrong image. Put them back to back, or save the URL to a named cache var first.
